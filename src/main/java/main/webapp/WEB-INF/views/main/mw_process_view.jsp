...
<body>
	...
	<c:set var="defaultTitle" value="제목 : 내용 : " />
	<div class="wrapper main-page">

		<div class="container">
			<h3 class="col-md-4">이력관리</h3>
			<form id="updateComplaint" method="post">
				<input type="hidden" id="cId" name="cId" value="${processComplaint.cId }">
				 <input
					type="hidden" id="cDepartment" name="cDepartment"
					value="${department }"> 
				<input type="hidden" name="cAdminNum" id="cAdminNum"
					...
					<tbody>
						<tr>
							...
							<td><select name="cSeparationType" id="cSeparationType">
									<option>${processComplaint.cSeparationType }</option>
									<option>도로교통</option>
									<option>보행환경</option>
									<option>생활안전</option>
									<option>주차환경</option>
						...
						<tr>
							...
							<td rowspan="2" colspan="3"><textarea name="cTitle"
									...
									placeholder="제목(200자 이내)">${processComplaint.cTitle }</textarea></td>
						...
						<tr>
							...
							<td colspan="3" rowspan="2"><textarea name="cAddress"
									...
									placeholder="위치(100자 이내)">${processComplaint.cAddress }</textarea></td>
							<th class="bg-light">행정동</th>

						</tr>
						<tr>
							<td><select name="cDistrict" id="cDistrict">
									<option>${processComplaint.cDistrict }</option>
									<option>충장동</option>
									<option>동명동</option>
									<option>계림1동</option>
									<option>계림2동</option>
						...
						<tr>
							<td><input type="date" id="cReceiptDate" name="cReceiptDate"
								value="${processComplaint.cReceiptDate }" max="9999-12-31"></td>
							<td><select name="cReceiptWay" id="cReceiptWay">
									<option>${processComplaint.cReceiptWay }</option>
									<option>국민신문고</option>
							...
							<td><input type="number" id="cRaiseCount" name="cRaiseCount"
								value="${processComplaint.cRaiseCount }" min="0"></td>
							<td><input type="date" id="cRaiseDate" name="cRaiseDate"
								value="${processComplaint.cRaiseDate }" max="9999-12-31"></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><input type="text" id="cOfficer01" name="cOfficer01"
								value="${processComplaint.cOfficer01 }" placeholder="과장명 입력"></td>
							<td><input type="text" id="cOfficer02" name="cOfficer02"
								value="${processComplaint.cOfficer02 }" placeholder="계장명 입력"></td>
							<td><input type="text" id="cOfficer03" name="cOfficer03"
								value="${processComplaint.cOfficer03 }" placeholder="담당자명 입력"></td>
						</tr>
						<tr>
						...
						<tr>
							<td><select name="dpName01" id="dpName01">
									<option>${processComplaint.dpName01 }</option>
									<option>없음</option>
							...
							<td><select name="dpName02" id="dpName02">
									<option>${processComplaint.dpName02 }</option>
									<option>없음</option>
									<c:forEach var="department" items="${relatedDepartments }">
										<option>${department }</option>
									</c:forEach>
							...
							<td><select name="dpName03" id="dpName03">
									<option>${processComplaint.dpName03 }</option>
									<option>없음</option>
									<c:forEach var="department" items="${relatedDepartments }">
										<option>${department }</option>
									</c:forEach>
							...
							<td><select name="dpName04" id="dpName04">
									<option>${processComplaint.dpName04 }</option>
									<option>없음</option>
									<c:forEach var="department" items="${relatedDepartments }">
										<option>${department }</option>
									</c:forEach>
						...
						<tr>
							...
							<td colspan="5"><textarea name="cKeyPoint" id="cKeyPoint"
									wrap=on width="100%" hight="100%" rows="5"
									placeholder="민원요지(200자 이내)">${processComplaint.cKeyPoint }</textarea></td>
						</tr>
						<tr>
							<th class="point"><span style="color: red;">*</span> 민원내용</th>
							<td colspan="5"><textarea name="cContent" id="cContent"
									wrap=on width="100%" hight="100%" rows="5"
									placeholder="민원내용 (2000자 이내)">${processComplaint.cContent }</textarea></td>
						</tr>
						<tr>
							<th class="point"><span style="color: red;">*</span> 처리계획</th>
							<td colspan="5"><textarea name="cPlan" id="cPlan" wrap=on
									width="100%" hight="100%" rows="5"
									placeholder="처리계획 (1000자 이내)">${processComplaint.cPlan }</textarea></td>
						</tr>
					</tbody>
				</table>
			</form>
				...
				<tbody>
					...
					<tr>
						<td colspan="2"><textarea name="cTitle" id="cTitle" wrap=on
								width="100%" hight="100%" placeholder="제목(200자 이내)">${processComplaint.cTitle }</textarea>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2"><textarea name="cAddress" id="cAddress"
								wrap=on width="100%" hight="100%" placeholder="위치(100자 이내)">${processComplaint.cAddress }</textarea></td>
					</tr>
					<tr>
						<th class="bg-light">행정동</th>
						<td><input type="text" id="cDistrict" name="cDistrict"
							placeholder="행정동(20자 이내)" value="${processComplaint.cDistrict }"></td>
					</tr>
					<tr>
					...
					<tr>
						<td><input type="text" id="cPetitioner" name="cPetitioner"
							placeholder="성명(20자 이내)" value="${processComplaint.cPetitioner }"></td>
						<td><input type="text" id="cPetitionerPhone"
							name="cPetitionerPhone" placeholder="연락처(20자 이내)"
							value="${processComplaint.cPetitionerPhone }"></td>
					</tr>
					...
					<tr>
						...
						<td><input type="text" id="cReceiptWay" name="cReceiptWay"
							placeholder="신청방법(20자 이내)"
							value="${processComplaint.cReceiptWay }"></td>
					</tr>
					<tr>
						<th class="bg-light">제기횟수(중복)</th>
						<th class="bg-light">최초제기일</th>
					...
					<tr>
						<td><input type="number" id="cRaiseCount" name="cRaiseCount"
							value="${processComplaint.cRaiseCount }"></td>
						<td><input type="date" id="cRaiseDate" name="cRaiseDate"
							value="${processComplaint.cRaiseDate }" max="9999-12-31"></td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2"><textarea name="cKeyPoint" id="cKeyPoint"
								wrap=on placeholder="민원요지(200자 이내)">${processComplaint.cKeyPoint }</textarea></td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2"><textarea name="cContent" id="cContent"
								wrap=on rows="5" placeholder="민원내용 (2000자 이내)">${processComplaint.cContent }</textarea></td>
					</tr>
					<tr>
					...
					<tr>
						<td colspan="2"><textarea name="cPlan" id="cPlan" wrap=on
								rows="5" placeholder="처리계획 (1000자 이내)">${processComplaint.cPlan }</textarea></td>
					</tr>
				</tbody>
			</table>
			<div class="button-wrapper">
				<button type="button" id="imsi" class="btn btn-secondary" onclick="updateComplaint()">수정 변경</button>
			</div>
			...
			<div id="div3">
				...
				<div class="border p-3 my-3 bg-light d-flex">
					...
					<button type="button" class="btn btn-bule" id="btnInsertProcess"
						name="btnInsertProcess">처리등록</button>
				</div>
								...
								<c:choose>
									<c:when test="${sessionScope.userId == settle.uId }">
										<form id="settleUpdate${settle.csId }" name="settleUpdate">
											<input type="hidden" id="cId" name="cId"
												value="${processComplaint.cId }"> <input
												type="hidden" id="csId" name="csId" value="${settle.csId }" />
											...
											<td><img src="${path}/resources/images/edit.png" onclick="updateSettle(${settle.csId})" style="cursor: pointer"/></td>
											<td><img src="${path}/resources/images/trash.png" onclick="deleteSettle(${settle.csId})" style="cursor: pointer"/>
											</td>
											</td>
										</form>
				...
				<c:if test="${fn:length(complaintSettles) == 0}">
					<div class="d-flex justify-content-center mb-3">해당 항목이 존재하지 않습니다.</div>
				</c:if>
			</div>
				...
				<form id="insertImageForm" method="post"
					...
					<div class="border p-3 my-3 bg-light d-flex">
						...
						<input type="hidden" id="cId" name="cId"
							...
							type="hidden" id="uName" name="uName"
							value="${processComplaint.cOfficer03 }"> <input
							type="hidden" id="psName" name="psName" value="#">
	                  ...
	                  >
	                   -->
						...
						<button type="button" class="btn btn-bule" id="btnInsertImage"
							name="btnInsertImage">업로드</button>
				</form>
			</div>
			...
			<form id="insertFileForm" method="post" encType="multipart/form-data">
				<div class="border p-3 my-3 bg-light d-flex">
					<input type="hidden" id="cId" name="cId"
						...
						id="uId" name="uId" value="${sessionScope.userId }"><input
						type="hidden" id="uName" name="uName"
						value="${processComplaint.cOfficer03 }">
					<div class="mr-2" style="width: calc(100% - 74px)">
						<div class="input-group">
							<div class="custom-file">
	...
	<div id="div2" style="background: #f3f7fa; padding: 10px;">
		...
		<form id="updateProcessForm" method="post">
			<input type="hidden" id="cId" name="cId"
				...
				value="${processComplaint.cNotificated }">
				...
				<tbody>
					<tr>
						<th class="border-blue point"><span style="color: red;">*</span>
							조치내용(민원답변)</th>
						<td colspan="4" class="bg-white"><textarea name="cAnswer" id="cAnswer"
								wrap="on" rows="5" placeholder="조치내용(민원답변)"></textarea>
						</td>
					</tr>
					...
					<tr>
						<th class="bg-light"><span style="color: red;">*</span>
							완료(예정)일</th>
						<th class="bg-light"><span style="color: red;">*</span> 민원답변완료</th>
						<th class="bg-light"><span style="color: red;">*</span> 민원 만족도</th>
						<th class="bg-light" colspan="2"><span style="color: red;">*</span>
							비고</th>
					</tr>
					<tr>
						<td class="bg-white"><input type="date" id="cDeadline" name="cDeadline"
							value="${processComplaint.cDeadline }" max="9999-12-31"></td>
						...
						<!-- 
	                <td><input type="text" id="cSatisfiction" name="cSatisfiction" value="${processComplaint.cSatisfiction }"></td>
	                 -->
						<td class="bg-white"><select name="cSatisfiction" id="cSatisfiction">
								<option>선택</option>
								<option>매우만족</option>
								<option>만족</option>
								<option>보통</option>
						...
						<td class="bg-white"><input type="text" id="cClient" name="cClient"
							value="${processComplaint.cClient }"></td>
					</tr>
					<tr>
						<th class="point">참고사항</th>
						<td colspan="4" class="bg-white"><input type="text" id="cReference"
							name="cReference" value="${processComplaint.cReference }"></td>
					</tr>
				</tbody>
			</table>
		</form>
			...
			<tbody>
				...
				<tr>
					<td colspan="2"><textarea name="cAnswer" id="cAnswer"
							wrap="on" rows="5" placeholder="조치내용(민원답변)">${processComplaint.cAnswer }</textarea>
					</td>
				</tr>
				<tr>
	...
	<script>
				...
				.ready(
						function() {
							$("#btnTypeUpdate")
									.click(
											function() {
												$("#updateTypeForm")
														.attr("action",
																"/main/update_separation_type.do");
												$("#updateTypeForm").submit();
											});
							
							$("#btnPlanUpdate").click(function() {
								var plan = $("#cPlan").val();
								if (plan == "") {
									alert("처리계획을 입력해주세요.");
									return;
								...
								if (confirm("변경하시겠습니까?")) {
									$("#updatePlanForm").attr("action", "/main/update_plan.do");
							...
							$("#btnInsertImage")
									.click(
											function() {
												let size = $("#insertImageForm")[0][5].files[0].size;
												let type = $("#insertImageForm")[0][5].files[0].type;
												console
														.log($("#insertImageForm"));
							...
							$("#btnInsertFile")
									.click(
											function() {
												let size = $("#insertFileForm")[0][3].files[0].size;
		...
		function updateComplaint() {
			...
			if (confirm("수정하시겠습니까?")) {
				$("#updateComplaint").attr("action",
				"update_complaint.do");
				$("#updateComplaint").submit();
			}
		}
		
		function complaintComplete() {
			console.log($("#cDepartment").val());
			var title = $("#cTitle").val();
			if (title == '') {
				alert("제목을 입력해주세요.");
		...
		function deleteImage(ciId) {
			if (confirm("삭제하시겠습니까?")) {
				location.href="/main/delete_image.do?id=" + ciId + "&cId=" + ${processComplaint.cId}; 
			}
		}
		
		function deleteFile(cfId) {
			if (confirm("삭제하시겠습니까?")) {
				location.href="/main/delete_file.do?id=" + cfId + "&cId=" + ${processComplaint.cId}; 
			}
		}
		
		function deleteReply(crId) {
			if (confirm("삭제하시겠습니까?")) {
				location.href="/main/delete_reply.do?id=" + crId + "&cId=" + ${processComplaint.cId}; 
			}
		}
	</script>
</body>
