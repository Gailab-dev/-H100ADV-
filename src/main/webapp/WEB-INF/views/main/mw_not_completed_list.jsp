...
<body>
		...
		<div class="container position-relative">
			...
			<div class="border px-3 pt-3 pb-1 my-3 bg-light">
				<div class="row mx-1">
					...
					<div class="col-lg-2 col-md-6 pb-2 ml-auto text-right">진행중인
			...
			<table class="table table-list table-lg border-bottom mt-4 w-100">
				...
				<tbody>
					<c:forEach var="notCompletedComplaint" items="${processList }">
						<tr>
							<td data-label="신청접수일">${notCompletedComplaint.cReceiptDate }</td>
							<td data-label="제목">
								<p class="text-left text-truncate">
									<a href="mw_not_completed_view.do?id=${notCompletedComplaint.cId }">${notCompletedComplaint.cTitle }</a>
								</p>
							</td>
							<td data-label="담당부서">${notCompletedComplaint.dpMasterName }</td>
							<td data-label="행정동">${notCompletedComplaint.cDistrict }</td>
							<td data-label="분류유형">${notCompletedComplaint.cSeparationType }</td>
							<td data-label="처리구분">${notCompletedComplaint.cProcessClass }</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
	...
	<script>
		...
		function fn_prev(page, range, rangeSize, listSize, searchType, keyword,
			...
			var url = "/main/mw_not_completed_list.do";
			url += "?page=" + page;
			url += "&range=" + range;
