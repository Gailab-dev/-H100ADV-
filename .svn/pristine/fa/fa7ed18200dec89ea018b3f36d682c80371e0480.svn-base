...
<%
application.setAttribute("page", "mw_notice");
%>
<!DOCTYPE html>
<html lang="kr">
<head>
...
<body>
		...
		<div class="container position-relative">
				...
				<table class="table table-list table-lg border-bottom mt-4 w-100">
					...
					<tbody>
						<c:forEach items="${recentNoticeList }" var="notice">
							<tr>
								<td data-label="No." class="notice-tag"><span>공지사항</span></td>
								<td data-label="제목"><a
									href="/main/mw_notice_view.do?id=${notice.n_id }"><p
											class="text-left text-truncate">${notice.n_title }</p></a></td>
								<td data-label="파일첨부" class="notice-file"></td>
								<td data-label="[작성부서]작성자">[${notice.dp_name }] ${notice.u_name }</td>
								<td data-label="작성일">${notice.date }</td>
								<td data-label="조회수">${notice.n_count }</td>

							</tr>
						</c:forEach>
						<c:forEach items="${noticeList }" var="notice">
							<tr>
								<td data-label="No.">${notice.n_id }</td>
								<td data-label="제목">
									<a
									href="/main/mw_notice_view.do?id=${notice.n_id }"><p
											class="text-left text-truncate">${notice.n_title }</p></a>
								</td>
								<td data-label="파일첨부" class="notice-file"></td>
								<td data-label="[작성부서]작성자">[${notice.dp_name }] ${notice.u_name }</td>
								<td data-label="작성일">${notice.date }</td>
			...
			<div class="d-flex">	
				<button class="btn ml-auto btn-bule"
					onclick='location.href="/main/mw_notice_insert.do"'>공지사항
					작성</button>
			</div>
	...
	<script>
		...
		function fn_prev(page, range, rangeSize, listSize, searchType, keyword) {
			...
			var url = "/main/mw_notice_board.do";
			url += "?page=" + page;
			url += "&range=" + range;
